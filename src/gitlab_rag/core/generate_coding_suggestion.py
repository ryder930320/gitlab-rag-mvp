...
# 統一的 NIM 速率限制參數（與 hybrid_search.py 對齊）
GENERATE_MAX_RETRIES = 3
GENERATE_RETRY_BASE_DELAY = 10.0  # 秒，指數退避基準


def call_nim_generate(prompt: str, timeout: float = 120.0, max_retries: int = GENERATE_MAX_RETRIES, retry_base_delay: float = GENERATE_RETRY_BASE_DELAY) -> str:
    """
    呼叫 NIM 生成模型 API（統一速率限制邏輯：讀取 Retry-After，否則指數退避）。

    Args:
        prompt: 完整 prompt 字串
        timeout: 請求逾時秒數
        max_retries: 最大重試次數（預設 3 次）
        retry_base_delay: 指數退避基準秒數（預設 10 秒，針對 429）
    ...
    for attempt in range(max_retries + 1):
        ...
        try:
            with httpx.Client(timeout=timeout) as client:
                ...
                if resp.status_code == 402:
                    log_nim_call(
                        ...
                        response_payload={"error": "quota exhausted", "status_code": 402},
                        query=prompt[:200],
                        fallback_triggered=False,
                        latency_ms=latency_ms,
                        status_code=402,
                        error="HTTP 402 quota exhausted"
                    )
                    raise RuntimeError("NIM API 額度用盡或帳單問題 (HTTP 402)")
                if resp.status_code == 429:
                    retry_after = resp.headers.get("Retry-After")
                    wait_time = int(retry_after) if retry_after and retry_after.isdigit() else retry_base_delay * (2 ** attempt)
                    ...
                    if attempt < max_retries:
                        print(f"  ⚠️  NIM API 速率限制 (HTTP 429)，等待 {wait_time} 秒後重試... (第 {attempt + 1}/{max_retries} 次)")
                        ...
                        raise RuntimeError(f"NIM API 速率限制 (HTTP 429)，已重試 {max_retries} 次仍失敗")
                if resp.status_code == 503:
                    # 503 也應重試（CP-31 統一速率限制邏輯）
                    retry_after = resp.headers.get("Retry-After")
                    wait_time = int(retry_after) if retry_after and retry_after.isdigit() else retry_base_delay * (2 ** attempt)
                    fallback_triggered = True
                    log_nim_call(
                    ...
                    if attempt < max_retries:
                        print(f"  ⚠️  NIM API 服務不可用 (HTTP 503)，等待 {wait_time} 秒後重試... (第 {attempt + 1}/{max_retries} 次)")
                        time.sleep(wait_time)
                        continue
                    else:
                        log_nim_call(
                        ...
                        raise RuntimeError(f"NIM API 服務不可用 (HTTP 503)，已重試 {max_retries} 次仍失敗")
                ...
                log_nim_call(
                    ...
                    model=NIM_GENERATE_MODEL,
                    request_payload=payload,
                    response_payload={"finish_reason": data["choices"][0].get("finish_reason"), "completion_tokens": data.get("usage", {}).get("completion_tokens")},
                    query=prompt[:200],
            ...
            latency_ms = int((time.time() - start_time) * 1000)
            last_error = f"NIM API 請求逾時 ({timeout} 秒)"
            # timeout 也應重試
            fallback_triggered = True
            log_nim_call(
                call_type="generate",
            ...
            if attempt < max_retries:
                ...
                print(f"  ⚠️  NIM API 逾時，等待 {wait_time} 秒後重試... (第 {attempt + 1}/{max_retries} 次)")
                time.sleep(wait_time)
                continue
            else:
                log_nim_call(
                ...
                raise RuntimeError(last_error)
                    ...
                    latency_ms = int((time.time() - start_time) * 1000)
                    last_error = f"NIM API HTTP 錯誤: {e.response.status_code} - {e.response.text}"
                    # 503 在 try 區塊已處理，這裡處理其他 HTTP 錯誤（不重試）
                    log_nim_call(
                        call_type="generate",
                        model=NIM_GENERATE_MODEL,
                        request_payload=payload,
