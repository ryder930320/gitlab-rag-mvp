...
## [2025-07-21] - 階段延遲追蹤 + SSE 串流 + 前端即時顯示 + 雙重執行修復
...
- 修正 SSE 生成器縮進錯誤
- **關鍵修復：移除 `/suggest/timing` 端點雙重執行 bug** — 原代碼先手動跑完 pipeline 計時，再呼叫 `get_coding_suggestion()` 跑第二遍，只計時第一遍（常 timeout 斷點），回傳第二遍結果但 timing 是第一遍的（1.8s 假象）。現改為單次執行，直接用手動計時結果組裝回傳。
- **關鍵修復：SSE 串流 token 解析錯誤** — NIM streaming 先送 `reasoning_content` 才送 `content`，原代碼只看 `content` 導致前幾十個 token 全丟失；`except: pass` 靜默吞掉所有 parse 錯誤。現改為同時檢查 `content` 和 `reasoning_content`，具體捕獲 `json.JSONDecodeError, KeyError, IndexError` 並 log 警告不中斷。
- **關鍵修復：SSE 生成器結尾重複呼叫 `get_coding_suggestion()`** — 串流結束後又跑一遍完整 pipeline 只是為了拿 sources，現改為直接用已取得的 `raw_results` 組裝 sources。
- **前端 `rag_ui.html` 串流期間內容消失** — `handleStreamSubmit` 一開始 `resultSection.classList.remove("visible")` 隱藏整個結果區塊，導致 token 串流期間完全看不見內容，直到 `complete` 事件才顯示。現改為 `classList.add("visible")` 讓結果區塊一開始就顯示，token 即時可見。
